#!/bin/sh
set -e

CONFIG_TEMPLATE="/home/node/config.template.json"
CONFIG_TARGET="/home/node/.openclaw/openclaw.json"

# Ensure .openclaw directory is writable by node user
chown -R node:node /home/node/.openclaw 2>/dev/null || true

# Fallback: use TELEGRAM_BOT_TOKEN if OPENCLAW-specific not set
BOT_TOKEN="${OPENCLAW_TELEGRAM_BOT_TOKEN:-${TELEGRAM_BOT_TOKEN}}"
OWNER_ID="${OPENCLAW_TELEGRAM_OWNER_ID:-${TELEGRAM_CHAT_ID}}"

# Substitute environment variables in config template
if [ -f "$CONFIG_TEMPLATE" ]; then
  sed \
    -e "s|\${BOT_TOKEN}|${BOT_TOKEN}|g" \
    -e "s|\${OWNER_ID}|${OWNER_ID}|g" \
    "$CONFIG_TEMPLATE" > "$CONFIG_TARGET"
  chown node:node "$CONFIG_TARGET"
  echo "Config generated with env vars substituted."
else
  echo "WARNING: No config template found at $CONFIG_TEMPLATE"
fi

# Copy SOUL.md to workspace if available
if [ -f "/home/node/workspace-template/SOUL.md" ]; then
  cp /home/node/workspace-template/SOUL.md /home/node/.openclaw/workspace/SOUL.md 2>/dev/null || true
fi

# Drop privileges and exec openclaw as node user
exec gosu node openclaw "$@"
