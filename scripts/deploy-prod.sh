...
docker compose -f "$COMPOSE_FILE" build openclaw-gateway 2>/dev/null || true
docker compose -f "$COMPOSE_FILE" up -d --force-recreate --no-deps backend cotizar-api nginx openclaw-gateway
