...
logger = logging.getLogger("scraper.comprar_gob_ar")

class ComprarGobArScraper(BaseScraper):
    """Scraper for Comprar.gob.ar"""
    ...
    async def extract_licitacion_data(self, html: str, url: str) -> Optional[LicitacionCreate]:
        ...
        try:
            ...
            title_elem = soup.select_one(self.config.selectors.get("title", "h1.titulo"))
            if not title_elem:
                return None
            ...
            # Attached files
            attached_files = []
            file_elems = soup.select(self.config.selectors.get("attached_files", "div.archivos a"))
    ...
    async def extract_links(self, html: str) -> List[str]:
        ...
        link_elems = soup.select(self.config.selectors.get("links", "table.items a.ver-detalle"))
