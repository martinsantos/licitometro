...
Login endpoint: servlet/hccat004 (redirected from mpcatalogo)
  Fields: vCONUSRPRF (user, uppercase), vPASSWORD (password)
  Submit: BOTONING (data-gx-evt="5")
...
class ComprasAppsAuthClient:
    ...
    async def login(self) -> bool:
        """Login to ComprasApps via hccat004 servlet."""
        ...
        try:
            # Step 1: GET login page (mpcatalogo redirects to hccat004)
            login_url = f"{BASE_URL}/mpcatalogo"
            async with self._session.get(login_url, allow_redirects=True) as resp:
                if resp.status != 200:
                    logger.warning(f"ComprasApps login page returned {resp.status}")
                    return False
            ...
            fields["vCONUSRPRF"] = self.user.upper()  # GeneXus uppercases this field
            fields["vPASSWORD"] = self.password
            fields["_EventName"] = "E'BOTONING'."  # GeneXus button event

            await asyncio.sleep(self.AUTH_DELAY)

            # Step 3: POST login — form action is hccat004
            post_url = f"{BASE_URL}/hccat004"
            async with self._session.post(post_url, data=fields, allow_redirects=True) as resp:
                if resp.status != 200:
                    logger.warning(f"ComprasApps login POST returned {resp.status}")
                    await self._update_status(f"ERROR - HTTP {resp.status}")
                    return False
                body = (await resp.read()).decode("utf-8", errors="replace")

            # Check for success: should NOT show login form again
            if "vCONUSRPRF" in body and "Ingreso de Usuario" in body:
                # Still on login page — failed
                # Check for error message
                soup2 = BeautifulSoup(body, "html.parser")
                msg_div = soup2.find("div", id="MENSAJE")
                err_msg = msg_div.get_text(strip=True) if msg_div else "Unknown error"
                logger.warning(f"ComprasApps login failed: {err_msg}")
                await self._update_status(f"ERROR - {err_msg[:80]}")
                return False

            logger.info("ComprasApps login successful")
            ...
            await self._update_status(f"ERROR - {type(e).__name__}")
            return False
